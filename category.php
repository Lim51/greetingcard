...
$all_menu = $conn->query("SELECT id FROM card_list WHERE category_id = '{$cid}' AND status = 1")->num_rows;
$page_btn_count = ceil($all_menu / $limit);
$qry = $conn->query("SELECT * FROM card_list WHERE category_id = '{$cid}' AND status = 1 ORDER BY name ASC LIMIT $limit OFFSET $offset");
?>

<section class="page-section" id="menu">
    <h1 class="text-center text-cursive" style="font-size:3em"><b><?= $data['name'] ?? "" ?>'s Card</b></h1>
