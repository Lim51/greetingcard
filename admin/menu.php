						...
						<div class="card-body">
							...
							<div class="form-group">
								...
								<select name="category_id" id="" class="custom-select browser-default">
									<?php
									$cat = $conn->query("SELECT * FROM category_list WHERE status = 1 ORDER BY name ASC");
						...
						<div class="table-responsive">
							<table class="table table-bordered table-hover">
								...
								<tbody>
									<?php
									$i = 1;
									$cats = $conn->query("SELECT p.*, c.name as cat FROM card_list p inner join category_list c on c.id = p.category_id order by p.id asc");
									...
									?>
										<tr>
											...
											<td class="text-center">
												<button class="btn btn-sm btn-primary edit_menu" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-status="<?php echo $row['status'] ?>" data-description="<?php echo $row['description'] ?>" data-category_id="<?php echo $row['category_id'] ?>" data-img_path="<?php echo $row['img_path'] ?>">Edit</button>
